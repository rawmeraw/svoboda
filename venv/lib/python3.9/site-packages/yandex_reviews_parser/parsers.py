...
class Parser:
    ...
    def __scroll_to_bottom(self, elem, max_iter: int = 50, current: int = 0) -> None:
        if current >= max_iter:
            return
        ...
        if elem == new_elem:
            return
        self.__scroll_to_bottom(new_elem, max_iter, current + 1)
    ...
    def __get_data_item(self, elem):
        ...
        try:
            more_btns = elem.find_elements(By.XPATH, ".//*[contains(@class,'expand') or contains(@class,'more') or contains(@class,'show-more')]")
            if not more_btns:
                more_btns = elem.find_elements(By.XPATH, ".//button[.='ещё' or .='Читать полностью' or .='Показать полностью']")
            for btn in more_btns:
                try:
                    self.driver.execute_script("arguments[0].click(); arguments[0].remove();", btn)
                    time.sleep(0.3)
                except:
                    pass
            text = elem.find_element(By.XPATH, ".//div[@class='business-review-view__body']").text
            if text:
                text = text.rstrip('…').rstrip(' \n\r\t')
        except NoSuchElementException:
            text = None
        try:
            stars = elem.find_elements(By.CSS_SELECTOR, "[class*='rating-badge-view__stars'] > span")
